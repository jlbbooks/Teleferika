// profile_chart.dart
// Reusable profile chart: elevation or plan (lateral offset) with axes, labels, resize.
...
/// Mode of the profile chart: elevation (altitude vs distance) or plan (lateral offset).
enum ProfileChartMode { elevation, plan }

/// Data for the elevation profile: distances, altitudes, and Y-axis bounds.
class ProfileData {
...
/// Data for the plan (top-view) profile: along distances and lateral offsets.
class PlanProfileData {
...
/// Expandable profile chart with resize handle; elevation or plan mode.
class ExpandableProfileChart extends StatefulWidget {
  final ProjectModel project;
  final ProfileChartMode mode;
  final ProfileData? profileData;
  final PlanProfileData? planProfileData;
  final List<PointModel> points;

  const ExpandableProfileChart({
    super.key,
  ...
  @override
  State<ExpandableProfileChart> createState() => _ExpandableProfileChartState();
}

class _ExpandableProfileChartState extends State<ExpandableProfileChart> {
  ...
  double get _chartHeight => widget.mode == ProfileChartMode.elevation
  ...
  void _onResize(double delta) {
    ...
    setState(() {
      if (widget.mode == ProfileChartMode.elevation) {
      ...
      builder: (context, constraints) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                mode == ProfileChartMode.elevation
                    ? 'Elevation profile'
...
class _ProfilePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (mode == ProfileChartMode.elevation && profileData != null) {
    ...
    } else if (mode == ProfileChartMode.plan && planProfileData != null) {
      for (int i = 0; i < points.length; i++) {
      ...
      child: Stack(
        children: [
          for (int i = 0; i < points.length; i++) ...[
            ...
            if (mode == ProfileChartMode.plan && planProfileData != null)
              _labelAt(
